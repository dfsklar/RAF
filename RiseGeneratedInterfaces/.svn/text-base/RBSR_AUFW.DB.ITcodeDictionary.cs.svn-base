...
namespace RBSR_AUFW.DB.ITcodeDictionary
{
	...
	public class ITcodeDictionary
	{
		...
		public int NewTcodeDictionary(string TcodeID)
		{
			...
			if (TcodeID == null)  { cmd.Dispose(); DBClose(); throw new Exception("TcodeID must not be null!"); } 
		...
		public returnGetTcodeDictionary GetTcodeDictionary(int ID)
		{
			...
			while (dr.Read())
			{
				if (dr.IsDBNull(0))
					...
					rv.ID = dr.GetInt32(0);
				if (dr.IsDBNull(1))
					 { cmd.Dispose(); DBClose(); throw new Exception("Value 'null' is not allowed for 'TcodeID'"); } 
				else
