...
function MENUACTnewblank()
{
	...
	= "DLOGselectAuthObj.aspx?RoleID=" + $F('RoleID') + "&origin=" + escape(window.location.href);
}
...
function JDLGeditRow_Launch(IDtass)
{
    ...
    if (rightclickItem) {
	$('DLGCTL_c_u_ObjName').value = rightclickItem.getMember('SAPauthobjname').Value;
	$('DLGCTL_c_u_FieldName').value = rightclickItem.getMember('SAPauthfieldname').Value;
