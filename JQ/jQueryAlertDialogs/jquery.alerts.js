...
(function($JQ) {
	...
	$JQ.alerts = {
		...
		alert: function(message, title, callback) {
			...
			$JQ.alerts._show(title, message, null, 'alert', function(result) {
		...
		// DFSKLAR ADDED:
		multilineprompt: function(message, value, title, callback) {
			if( title == null ) title = 'Prompt';
			$JQ.alerts._show(title, message, value, 'multilineprompt', function(result) {
				if( callback ) callback(result);
			});
		},
		...
		_show: function(title, msg, value, type, callback) {
			...
			switch( type ) {
				case 'alert':
					$JQ("#popup_message").after('<div id="popup_panel"><input type="button" value="' + $JQ.alerts.okButton + '" id="popup_ok" /></div>');
				...
				break;
				case 'confirm':
					$JQ("#popup_message").after('<div id="popup_panel"><input type="button" value="' + $JQ.alerts.okButton + '" id="popup_ok" /> <input type="button" value="' + $JQ.alerts.cancelButton + '" id="popup_cancel" /></div>');
					$JQ("#popup_ok").click( function() {
						$JQ.alerts._hide();
				...
				case 'multilineprompt':
					$JQ("#popup_message").append('<br /><textarea cols="55" rows="10" id="popup_prompt"></textarea>').after('<div id="popup_panel"><input type="button" value="' + $JQ.alerts.okButton + '" id="popup_ok" /> <input type="button" value="' + $JQ.alerts.cancelButton + '" id="popup_cancel" /></div>');
	...
	jAlert = function(message, title, callback) {
		$JQ.alerts.alert(message, title, callback);
	}
